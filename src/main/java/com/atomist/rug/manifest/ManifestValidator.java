...
public abstract class ManifestValidator {
    ...
    public static Manifest validate(Manifest manifest) {
        if (manifest.group() == null || manifest.group().length() == 0) {
            throw new ManifestException(
                    "group should not be empty. Please correct group in manifest.yml/package.json");
        }
        if (manifest.artifact() == null || manifest.artifact().length() == 0) {
            throw new ManifestException(
                    "artifact should not be empty. Please correct artifact in manifest.yml/package.json");
        }
    ...
    private static void validateVersion(String version, String key) {
        if (version == null) {
            throw new ManifestException(
                    "%s should not be empty. Please correct %s in manifest.yml/package.json", key,
                    key);
        }
        ...
        try {
            ...
            throw new ManifestException(
                    "%s is not a valid version/version range. Please correct %s in manifest.yml/package.json",
                    version, key);
        }
    }
